...
@section('content')
	@section('actions')
		<a href="{{ '/admin/projects/' . $project->id . '/edit' }}" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold"> {{ __('core::app.btn_back') }}</a>
		<a href="{{ route('projects.index') }}" class="btn btn-flex btn-outline btn-color-gray-700 btn-active-color-primary bg-body h-40px fs-7 fw-bold">{{ __('core::app.btn_to_list') }}</a>
		...
		<div class="card-header border-1 pt-0">
			...
			<div class="card-title">
				...
				<div class="d-flex align-items-center position-relative my-1">
					...
					<input type="text" data-kt-customer-table-filter="search" class="form-control w-250px ps-13 rounded-0 h-40px" placeholder="{{ __('core::app.placeholder_search') }}" />
				</div>
				<!--end::Search-->
			</div>
			<!--begin::Card title-->
