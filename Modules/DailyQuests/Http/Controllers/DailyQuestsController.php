...
class DailyQuestsController extends CrudController
{
	protected $isSortable = true;
    protected $outlist = 'datatables';
    protected $templateIndex = 'daily_quests::daily_quests.index';
	protected $templateEdit = 'daily_quests::daily_quests.edit';
    protected $titles = ['Daily Quests', 'Daily Quest'];
    ...
    public function edit($id)
    {
        ...
        for ($i = 0; $i < $days_count; $i++) {
            ...
            $completed = DB::selectOne('
                SELECT COUNT(id) as count 
                FROM accounts_daily_quests
                WHERE daily_quest_id = ' . $id . '
                AND status = 1  
                AND updated_at BETWEEN ? AND ?
            ', [$startOfDay, $endOfDay])->count;

            $incomplete = DB::selectOne('
                SELECT COUNT(id) as count 
                FROM accounts_daily_quests
                WHERE daily_quest_id = ' . $id . '
                AND status = 0  
                AND amount > 0
                AND amount != required_amount
                AND updated_at BETWEEN ? AND ?
            ', [$startOfDay, $endOfDay])->count;
...
//        $item = $this->crudService->getItemById($id)->load('partners')->load('certificates');
        ...
        $route = action($this->getActionRoute('update'), ['daily_quest' => $id]);
	...
	{
		$model = DailyQuest::findOrFail($id);
...
//            return view('core::list_fields.group', compact('model'));
        ...
        $datatable->addColumn('action', function($model){
            $controller = $this->getController();
            return view('daily_quests::list_fields.actions', compact('model', 'controller'));
        });
