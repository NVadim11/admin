	...
	(() => {
				...
				.then(function(res){
					if(res.result == false) {
						...
						if(res.user) {
							...
							loginBtn.outerHTML = '<a class="headerAlt__entryBtn" href="/user">РџСЂРёРІС–С‚,<span>&nbsp;' + res.user.login + '</span></a>';
							successRegToggler();
						}
					}
				})
		...
		function login(formElement) {
			...
			fetch(formElement.getAttribute('action'), {
				...
				.catch(function(res){ invalidUser.style.display = "flex"; })
		}
		...
		class Popup {
				...
				document.addEventListener("click", function(e) {
					...
					if (buttonOpen) {
						...
						if (this._dataValue !== "error") {
							...
							this.open();
							return;
						} else this.popupLogging(`Р™РѕР№, РЅРµ Р·Р°РїРѕРІРЅРµРЅРѕ Р°С‚СЂРёР±СѓС‚ Сѓ ${buttonOpen.classList}`);
						return;
					}
			...
			open(selectorValue) {
				if (bodyLockStatus) {
					...
					} else this.popupLogging(`Р™РѕР№, С‚Р°РєРѕРіРѕ РїРѕРїР°РїСѓ РЅРµРјР°С”. РџРµСЂРµРІС–СЂС‚Рµ РєРѕСЂРµРєС‚РЅС–СЃС‚СЊ РІРІРµРґРµРЅРЅСЏ. `);
				}
			}
			close(selectorValue) {
			...
			popupLogging(message) {
				this.options.logging ? functions_FLS(`[РџРѕРїР°РїРѕСЃ]: ${message}`) : null;
			}
		}
		class PopupLogin {
			constructor(options) {
		...
		let gotoBlock = (targetBlock, noHeader = false, speed = 500, offsetTop = 0) => {
			...
			if (targetBlockElement) {
				...
				functions_FLS(`[gotoBlock]: Р®С…СѓСѓ...С—РґРµРјРѕ РґРѕ ${targetBlock}`);
			} else functions_FLS(`[gotoBlock]: Р™РѕР№... РўР°РєРѕРіРѕ Р±Р»РѕРєСѓ РЅРµРјР°С” РЅР° СЃС‚РѕСЂС–РЅС†С–: ${targetBlock}`);
		};
		...
		function formSubmit() {
			...
			function formSent(form, responseResult = ``) {
				...
				forms_formValidate.formClean(form);
				formLogging(`Р¤РѕСЂРјСѓ РІС–РґРїСЂР°РІР»РµРЅРѕ!`);
			}
			function formLogging(message) {
				functions_FLS(`[Р¤РѕСЂРјРё]: ${message}`);
			}
		}
		class SelectConstructor {
			...
			selectsActions(e) {
				...
				if (targetElement.closest(this.getSelectClass(this.selectClasses.classSelect)) || targetElement.closest(this.getSelectClass(this.selectClasses.classSelectTag))) {
					...
					} else if (targetType === "keydown" && e.code === "Escape") this.selectsРЎlose();
				} else this.selectsРЎlose();
			}
			selectsРЎlose(selectOneGroup) {
				const selectsGroup = selectOneGroup ? selectOneGroup : document;
				const selectActiveItems = selectsGroup.querySelectorAll(`${this.getSelectClass(this.selectClasses.classSelect)}${this.getSelectClass(this.selectClasses.classSelectOpen)}`);
				if (selectActiveItems.length) selectActiveItems.forEach((selectActiveItem => {
					this.selectРЎlose(selectActiveItem);
				}));
			}
			selectРЎlose(selectItem) {
				const originalSelect = this.getSelectElement(selectItem).originalSelect;
			...
			selectAction(selectItem) {
				...
				if (originalSelect.closest("[data-one-select]")) {
					...
					this.selectsРЎlose(selectOneGroup);
				}
				setTimeout((() => {
					if (!selectOptions.classList.contains("_slide")) {
			...
			getSelectTitleValue(selectItem, originalSelect) {
				...
				if (originalSelect.hasAttribute("data-pseudo-label")) {
					pseudoAttribute = originalSelect.dataset.pseudoLabel ? ` data-pseudo-label="${originalSelect.dataset.pseudoLabel}"` : ` data-pseudo-label="Р—Р°РїРѕРІРЅС–С‚СЊ Р°С‚СЂРёР±СѓС‚"`;
		...
		function savePersonalData(formElement) {
			...
			const fileInput = document.querySelector('#profileEditPictureInput') ;
			const formData = new FormData(formElement);
			...
			fetch(formElement.getAttribute('action'), options)
				.then((response) => response.json())
				.then(function(res){
					if(res.result == 'invalid_password') {
						document.getElementById("personalsEdit__currentPass").classList.add("validationError");
						...
						return false;
					} else if(res.result == 'phone_exist') {
						editBusyNumber.style.display = "flex";
						document.getElementById("personalsEdit__phone").classList.add("validationError");
						document.getElementById("personalsEdit__phone").focus();
